...
fun IrSimpleFunction.addAnnotation(context: IrPluginContext, fqName: FqName, block: IrConstructorCallImpl.() -> Unit) {
    val annCls = context.referenceClass(fqName.classId)!!
    val annCon = annCls.constructors.first()
    val annConCallImpl = IrConstructorCallImpl.fromSymbolOwner(
        this.startOffset, this.endOffset,
    ...
    annConCallImpl.block()
    this.annotations += annConCallImpl
}