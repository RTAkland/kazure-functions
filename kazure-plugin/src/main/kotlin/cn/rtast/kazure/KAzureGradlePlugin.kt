...
import cn.rtast.kazure.next.MetadataCollectorTask
import kazure.next.BuildConfig
...
class KAzureGradlePlugin : KotlinCompilerPluginSupportPlugin {
    ...
    private fun registerTasks(project: Project) {
        ...
        val collector = project.tasks.register("runRouteCollector", MetadataCollectorTask::class.java) {
            it.group = "kazure"
            it.runtimeClasspath.from(
    ...
    private fun registerExtension(project: Project) {
        project.extensions.create("kazureNext", KAzureExtension::class.java)
        project.extensions.create("kembeddableNext", KEmbeddableResourcesExtension::class.java)
    }
    ...
    override fun apply(target: Project) {
        super.apply(target)
        target.dependencies.add("api", "kazure.next:kazure-framework:${BuildConfig.KOTLIN_PLUGIN_VERSION}")
